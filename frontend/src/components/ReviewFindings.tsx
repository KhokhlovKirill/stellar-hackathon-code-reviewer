  ...
  return (
    ...
    >
      ...
      >
        ...
        {f.short_label && (
          <span className="shrink-0 mt-0.5 text-xs px-2 py-1 rounded-md bg-white dark:bg-slate-950 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
            {f.short_label}
          </span>
        )}
              ...
              {result.findings.some((f) => f.short_label) && (
                <div className="mt-3 flex flex-wrap gap-1.5">
                  {result.findings
                    ...
                    .filter(Boolean)
                    .map((label) => (
                      <span key={label} className="text-xs rounded-md border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 py-1 text-slate-500 dark:text-slate-400">
                        {label}
                      </span>
                    ))}
                </div>

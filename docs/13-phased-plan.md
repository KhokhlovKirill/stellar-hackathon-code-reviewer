...
- [x] FastAPI `api/`: `POST /webhooks/{github,gitlab,bitbucket}`, `/healthz`,
      `/readyz`, `/metrics`
- [x] Верификация подписи: GitHub HMAC-256, GitLab token, Bitbucket secret/HMAC —
      по сырому телу, constant-time, fail-closed (C1)
- [x] Классификация события (PR opened/synchronize/reopened; MR open/update/reopen;
      Bitbucket created/updated; comment-события — задел под C7)
- [x] Идемпотентность: Redis по delivery-id + (repo,pr,head_sha), TTL 24ч
- [x] Постановка в очередь (Arq), ответ `202` < 1s; worker-task `run_scan(scan_id)`
- [x] `providers/base.py` + `github.py`/`gitlab.py`/`bitbucket.py`: `fetch_diff`
      (только изменённые файлы/hunks), парсинг unified diff (`unidiff`) →
      new_lineno + diff_position (C2)
- [x] Учёт в аналитике: added/removed lines, diff_bytes, est_tokens,
...
- [x] `pipeline/filter.py`: path/ext/glob (README/*.md, .gitignore, LICENSE,
      картинки/бинарь, локфайлы, vendored/generated, minified, size-cap),
      `.aegisignore`
- [x] Детект языка → выбор правил/промпт-профиля
- [x] Запись kept/skipped(+reason) в лог, scan-record и summary
...
- [x] Secrets: gitleaks-набор + энтропия, только по `+`-строкам, маскирование
- [x] Semgrep-обвязка: рулсеты в образе, запуск по изменённым файлам, фильтр находок
      на пересечение с изменёнными строками, маппинг → CWE/severity/line
- [x] SCA: парс diff манифестов/локфайлов → OSV.dev + GHSA + deps.dev; CVE/yanked/
      malware/typosquat; предложение замены по контексту импортов
- [x] Нормализация → `Finding(source=deterministic, confidence≈0.98)`; передача как
...
- [x] `llm/base.py` единый `complete(messages, schema, budget)` + usage/cost/redaction
- [x] `llm/openai_compat.py` + `llm/router.py` (`local-secure`=SFT+ORPO
      Qwen3-Coder-30B-A3B, `local-base`=Qwen 3.6 35B A3B 4-bit MLX),
      health-probe `/v1/models` (TTL 30s)
- [x] OpenRouter generalist/judge слоты через OpenAI-compatible client, ретраи, бюджеты
- [x] `llm/router.py`: профиль ансамбля, авто-fallback при недоступном тире,
      метрики tier-down/fallback
- [x] Override-промпт для дообученной модели (CTF→JSON) + parser + schema-валидация
...
- [x] Построение grounded-промпта: hunks + узкий контекст + det-findings + anti-injection
      (вкл. negative), anti-injection маркеры (`05 §4,§5`)
- [x] Параллельные детекторы A (don) + B (generalist)
- [x] Judge-арбитр: дедуп, анти-FP rubric, severity/confidence, fix
- [~] Гейтинг публикации + FP-learning suppression; update старых комментариев ещё впереди
...
- [x] `render.py`: inline-комментарий (severity-бейдж, CWE, «почему эксплуатируемо
      здесь», offending-сниппет) с привязкой к строке/position на каждом провайдере (C4)
- [x] Suggestion-блоки (one-click apply) + fallback fenced patch/text
- [x] Summary-комментарий: таблица по severity, scanned/skipped, модели, scan-id,
      ссылка в аналитику, приглашение к диалогу
- [x] `policy.py`: при Critical (политика репо) → request-changes + падающий required
...
- [x] Webhook comment-событий → фильтр (@mention/прямой reply)
- [x] Контекст треда → LLM/fallback → reply in-thread
- [~] Состояние треда сохраняется; max-turn/rate-limit hard guard ещё впереди
...
- [x] Регистрация репо (провайдер, токен, webhook-secret) через backend API
- [x] Token vault (Fernet/AES-GCM, ключ из env/KMS), redaction
- [x] Политика per-repo (severity-gate, merge-block, ignore-globs, ensemble-profile,
      prompt-overrides, lang, budgets), `admin_audit`
- [x] Backend endpoints: Repos, Scans, Scan Detail, Stats, auth token
...
- [~] `eval/run.py`: offline seed metrics P/R/line-acc; большой PrimeVul set впереди
...
- [x] CI регресс-гейт `python -m eval.run --gate`
